...
/**
 * Generate a full KDP wrap cover in one step
 * This combines front cover generation, spine creation, and back cover assembly
 */
export async function generateFullWrapCover({
  prompt,
  negative_prompt,
  trimSize,
  pageCount,
  ...
  try {
    console.log('Starting full wrap cover generation with params:', { 
      prompt, trimSize, pageCount, paperType, includeBleed 
    });
    ...
    console.log('Sending request to generate-full-wrap endpoint');
    const response = await fetch(`${API_URL}/book-cover/generate-full-wrap`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('Error response from generate-full-wrap:', errorText);
      ...
      throw new Error(errorData.error || 'Failed to generate full wrap cover');
    }

    const data = await response.json();
    console.log('Full wrap generation response data:', data);
    
    return {
      fullCoverUrl: data.fullCover,
    ...
    toast.error('Failed to generate full wrap cover');
    throw error;
  }
}
